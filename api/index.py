...
# Resolve dataset path inside the API folder
...
try:
    with open(CSV_PATH, 'r') as f:
        ...
        for row in reader:
            x_data.append(float(row['Screen_Time_Hours']))
            y_data.append(float(row['Battery_Remaining_Percent']))
except Exception as e:
    print(f"Error loading CSV dataset: {e}")

# Compute Linear Regression parameters mathematically: y = slope * x + intercept
count = len(x_data)
if count > 0:
    avg_screen_time = sum(x_data) / count
    avg_battery = sum(y_data) / count
    min_battery = min(y_data)
    max_battery = max(y_data)

    mean_x = avg_screen_time
    mean_y = avg_battery
    
    # Calculate slope and intercept
    num = sum((x_data[i] - mean_x) * (y_data[i] - mean_y) for i in range(count))
    den = sum((x_data[i] - mean_x) ** 2 for i in range(count))
    slope = num / den if den != 0 else 0
    intercept = mean_y - slope * mean_x

    # Calculate R2 score
    ss_tot = sum((y_data[i] - mean_y) ** 2 for i in range(count))
    ss_res = sum((y_data[i] - (slope * x_data[i] + intercept)) ** 2 for i in range(count))
    r2_score = 1 - (ss_res / ss_tot) if ss_tot != 0 else 1
else:
    avg_screen_time = 5.5
    avg_battery = 51.6
    min_battery = 13.4
...
def get_stats():
    return jsonify({
        ...
        'mean_screen_time': round(avg_screen_time, 2),
...
def get_chart_data():
    ...
    # Sort dataset bounds to generate regression line endpoints
    min_x = min(x_data) if x_data else 0.0
    max_x = max(x_data) if x_data else 10.0
    
    # Generate 15 points along the regression line
    line_x = [min_x + (max_x - min_x) * i / 14 for i in range(15)]
...
def predict():
    try:
        ...
        # Prediction calculation using slope/intercept
        prediction_val = slope * screen_time + intercept
        clamped_val = max(0.0, min(100.0, prediction_val))
        
        # Calculate confidence score
        dist_from_mean = abs(screen_time - avg_screen_time)
...
def chat():
    try:
        ...
        if parsed_hours is not None:
            ...
            pred = slope * parsed_hours + intercept
        ...
        if any(k in lower_msg for k in ['stats', 'statistics', 'dataset', 'mean', 'average', 'min', 'max', 'records', 'count']):
            reply = (
                f"📊 *[Dataset Insights]*\n"
                f"• Total records in testing set: **{count}**\n"
                f"• Average Screen Time: **{avg_screen_time:.2f} hours**\n"
                f"• Average Battery Remaining: **{avg_battery:.2f}%**\n"
                f"• Maximum Tested Battery: **{max_battery}%**\n"
                f"• Minimum Tested Battery: **{min_battery}%**\n\n"
...
def export_pdf():
    try:
        ...
        model_info = [
            ...
            [Paragraph("Dataset Records Trained:", bold_body), Paragraph(f"{count} records", body_style)],
            [Paragraph("Average Screen Time:", bold_body), Paragraph(f"{avg_screen_time:.2f} hours", body_style)]
        ]
